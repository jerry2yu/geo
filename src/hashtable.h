 ...
 * the hash set iterator.  Even though everything is
 ...
 * HashSetIterator using those functions defined in this file.
 * update: Jan 10, 2005 by Zheyuan Yu
 */

typedef struct HasSetIterator{
	int ArrayIndex;/* the current array of the iterator */
	int BucketIndex; /* the current bucket of the iterator */
	hashset *HashSet; /* hash set that the iterator holding */
	void *Elem; /* element that the iterator pointed to */
}  HashSetIterator;

/**
 * Function: HashSetIteratorNew
 * -------------
 * create a new iterator for hashtable h.
 * the internal field of iter will be initialized and return iterator iter
 * an assert will be raised if hashset h is NULL
 * the iterator need to be release by calling HashSetIteratorFree after used.
 *
 */
...
/**
 * Function: HashSetIteratorNext
 * -------------
 * move iterator to the next element in the hash table
 * if it has excceed the end of the hash table, will return false, otherwise return true
 * iter->elem will point to the element in the hash table
 * update: Jan 10, 2005 by Zheyuan Yu
 */
bool HashSetIteratorNext(HashSetIterator* iter);

/**
 * Function: HashSetIteratorFree
 * ---------------
 * free the space of iterator
 * update: Jan 10, 2005 by Zheyuan Yu
 */
